...
interface UploadStepProps {
  onNext: (file: File, separator: string, headers: string[]) => void;
}
const SEPARATORS = [
{
  label: 'Comma (,)',
    ...
    <motion.div
      ...
      <motion.div variants={itemVariants} className="text-center md:text-left">
        ...
        <p className="text-sm font-medium text-slate-400 mt-2">
          Select a CSV file to begin the seeding process.
        </p>
      </motion.div>
      ...
      <motion.div variants={itemVariants} className="flex justify-end mt-2">
        <motion.button
          whileHover={
          !file || headers.length === 0 || !!error || isLoading ?
          {} :
          {
            scale: 1.02
          }
          ...
          disabled={!file || headers.length === 0 || !!error || isLoading}
          onClick={() => file && onNext(file, separator, headers)}
