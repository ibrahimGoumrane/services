...
class WebSocketLogHandler(logging.Handler):
    """Buffer logs and forward them to WebSocket subscribers in batches for a specific job."""

    def __init__(self, job_id: str, event_loop: asyncio.AbstractEventLoop, max_size: int = 1) -> None:
    ...
    def emit(self, record: logging.LogRecord) -> None:
        try:
            ...
            print("Failed to buffer log record for WebSocket", flush=True)
    ...
    def flush(self) -> None:
        ...
        try:
            ...
            print("Failed to flush WebSocket log buffer", flush=True)


class BufferedFileHandler(logging.Handler):
    """Buffer formatted log lines and append them to disk in batches."""

    def __init__(self, file_path: str, max_size: int = 1) -> None:
...
def attach_websocket_log_handler(
    ...
    if not job_id:
        return None
    handler = WebSocketLogHandler(job_id=job_id, event_loop=event_loop, max_size=max_size)
